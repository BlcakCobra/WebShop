...
export const RequestesToServer = {
  ...
  async createProductDetails(token: string, productDetails: ProductDetailsType) {
    try {
      if (!token) {
        throw new Error("Authorization token is required.");
      }
      ...
      formData.append("availableForPreorder", String(availableForPreorder || false));
      if (videoReview) formData.append("videoReview", videoReview);

      subjectSizes.forEach(size => formData.append("subjectSizes[]", size));
      subjectColors.forEach(color => formData.append("subjectColors[]", color));

      images.forEach((file) => {
        formData.append("images", file);
      });

      const response = await BaseUrl.post(`/createProductDetails`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async SearchResultFilter(params: SearchFilterParams, page: number) {
    try {
      ...
      const response = await BaseUrl.get(`/searchResaultFilter?${queryString.toString()}&page=${encodeURIComponent(page)}`);
  ...
  async recommendationForDetail(productId: string) {
    if (!productId) {
      throw new Error("Product ID not found")
    }
    try {
      const response = await BaseUrl(`/recommendationForDetails${productId}`)
      if (response.status !== 200) {
        throw new Error("Something went wrong with Search Product");
      }
      return response.data
